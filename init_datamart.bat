...
docker exec -it %CONTAINER% psql -U %USER% -d %DB% -c "CREATE SCHEMA IF NOT EXISTS transaccional; CREATE SCHEMA IF NOT EXISTS estrella; SET search_path TO estrella, transaccional, public;"

echo ^>^>^> [2/5] Cargando base transaccional...
docker exec -it %CONTAINER% psql -U %USER% -d %DB% -v ON_ERROR_STOP=1 -f /scripts/base_datos_transaccional.sql

echo ^>^>^> [3/5] Creando estructura del DataMart (DDL)...
docker exec -it %CONTAINER% psql -U %USER% -d %DB% -v ON_ERROR_STOP=1 -f /scripts/1_dm.sql

echo ^>^>^> [4/5] Creando Vista G (logica analitica)...
docker exec -it %CONTAINER% psql -U %USER% -d %DB% -v ON_ERROR_STOP=1 -f /scripts/2_G_pasos.sql

echo ^>^>^> [5/5] Poblando dimensiones y hechos (ETL)...
docker exec -it %CONTAINER% psql -U %USER% -d %DB% -v ON_ERROR_STOP=1 -f /scripts/3_poblar.sql
