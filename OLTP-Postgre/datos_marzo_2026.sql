...
--  Período: Marzo 2026
--
--  EJECUTAR DESPUÉS DE: datos_febrero_2026.sql
-- ============================================================
...
-- Consumo: OP-2026-001 â†’ Ropero x12
...
-- Consumo: OP-2026-002 â†’ Ropero x15
INSERT INTO consumo_material (produccion_id, material_id, cantidad_consumida, costo_unitario)
SELECT p.id, m.id, cons.cantidad, cons.precio
FROM produccion p
JOIN orden_produccion op ON op.id = p.orden_produccion_id
...
-- Consumo: OP-2026-003 â†’ Cómoda x18
INSERT INTO consumo_material (produccion_id, material_id, cantidad_consumida, costo_unitario)
SELECT p.id, m.id, cons.cantidad, cons.precio
FROM produccion p
JOIN orden_produccion op ON op.id = p.orden_produccion_id
...
-- Consumo: OP-2026-004 â†’ Velador x35
INSERT INTO consumo_material (produccion_id, material_id, cantidad_consumida, costo_unitario)
SELECT p.id, m.id, cons.cantidad, cons.precio
FROM produccion p
JOIN orden_produccion op ON op.id = p.orden_produccion_id
...
-- Consumo: OP-2026-005 â†’ Comodín x15
INSERT INTO consumo_material (produccion_id, material_id, cantidad_consumida, costo_unitario)
SELECT p.id, m.id, cons.cantidad, cons.precio
FROM produccion p
JOIN orden_produccion op ON op.id = p.orden_produccion_id
...
-- Consumo: OP-2026-006 â†’ Ropero x10
INSERT INTO consumo_material (produccion_id, material_id, cantidad_consumida, costo_unitario)
SELECT p.id, m.id, cons.cantidad, cons.precio
FROM produccion p
JOIN orden_produccion op ON op.id = p.orden_produccion_id
...
-- 23-Mar: Muebles El Norte (mayorista contado) â€” Ropero x8
WITH v AS (INSERT INTO venta (fecha,cliente_id,tipo_venta_id,usuario_id)
...
-- ============================================================
--  15. PAGOS DE CLIENTES â€” abonos registrados en marzo
-- ============================================================
